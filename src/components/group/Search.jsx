...
function Search({ type, isLoading, searchResults, handleClick, search }) {
  ...
  return (
    <div className={`${search ? "scrollbar-hide overflow-y-scroll h-[250px] mb-5  flex flex-col gap-y-3 pt-3" : "hidden"}`}>

      {
        isLoading ? <SkeletonLoading height={55} count={3} /> : (
          searchResults.length > 0 ? searchResults?.map((e) => {
            return (
              <div key={e._id} className='flex items-center justify-between bg-gray-300 rounded-lg p-4'>
                ...
                <button onClick={() => handleClick(e)} className='bg-green-600 rounded-xl px-3 py-2 text-[10.6px] tracking-wide text-[#fff]'>Add</button>
              </div>
            )
